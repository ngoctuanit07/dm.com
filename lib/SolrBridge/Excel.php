 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class SolrBridge_Excel extends Zend_Search_Lucene_Document_OpenXml {
    /**
    ...
    private function __construct($fileName, $storeContent)
    {
        ...
        foreach ($relations->Relationship as $rel) {
            if ($rel["Type"] == Zend_Search_Lucene_Document_OpenXml::SCHEMA_OFFICEDOCUMENT) {
                ...
                // Read shared strings
                $sharedStringsPath = $workbookRelations->xpath("rel:Relationship[@Type='" . SolrBridge_Excel::SCHEMA_SHAREDSTRINGS . "']");
                ...
                foreach ($workbookRelations->Relationship as $workbookRelation) {
                    if ($workbookRelation["Type"] == SolrBridge_Excel::SCHEMA_WORKSHEETRELATION) {
                    		...
                    		if('sku' !== $headData[$i]) {
                    			...
                    			if($attr) {
                    				...
                    				if ('date' === $frontendInput) {
                    					$time = SolrBridge_Excel_Date::ExcelToPHP($value);
     ...
     * @param string  $fileName
     * @param boolean $storeContent
     * @return SolrBridge_Excel
     */
    public static function loadXlsxFile($fileName, $storeContent = false)
    {
      	return new SolrBridge_Excel($fileName, $storeContent);
    }
