...
Product.Config.prototype.reloadPrice = function() {
    ...
    if(childProductId){
        ...
        //this.updateFormProductId(childProductId); comment by nhat
        this.addParentProductIdToCartForm(this.config.productId);
        this.showCustomOptionsBlock(childProductId, this.config.productId);
        this.showSpecification(childProductId, this.config.productId);
        this.addToQuote(childProductId, this.config.productId);
        if (usingZoomer) {
        ...
        this.showSpecification(false, false);
        this.addToQuote(false, false);
        if (usingZoomer) {
...
Product.Config.prototype.showSpecification = function(productId, parentId) {
	if(productId) {
		var specifications = this.config.childProducts[productId].specification;
		if (specifications.length) {
			var divWrapSpec = new Element('div');
			specifications.each(function(item){
				if(item.label === 'Description') {
					$('sbproduct-description').update(item.value);
				}
				else {
					var pEle = new Element('p').update('<span style="width: 20%; float: left;">'+item.label+': </span>' + item.value);
					divWrapSpec.appendChild(pEle);
				}
			});
			$('specification-wrapper').update(divWrapSpec);
		}
	}
}

Product.Config.prototype.addToQuote = function(productId, parentId) {
	$('button-quote').stopObserving('click');
	$('button-quote').observe('click', function(event){
		if(productId) {
			//show modal
			...
			var redirectUrl = location.protocol + "//" + location.host + '/sbquote/quote/addquote';
			...
			alert("Please specify the product's option(s).");
			Event.stop(event);
		}
	});
}
